...
const Cards = () => {
  const cardData = [
    ...
    },
    {
      ...
      'body': [
        ...
        <FlavorText>"It wasn't until the torch cast light that they realized they weren't
          alone."<br/>—Buddha, probably</FlavorText>,
      ],
    },
    {
      name: 'Longsword',
      ...
      body: [
        ...
        'When used to attack, roll ', <D10/>,
        ' equal to your Strength with a difficulty of 6 to attack the target for 3 damage.',
      ],
    },
    {
      name: 'Platemail',
  ...
  const cardsToRender = [...cardData, ...(cards || [])].map((card) => <SpecialCard {...card} key={card['id']}/>);

  const cardsWithMargin = cardsToRender.map((card) => <div style={{margin: '.5em'}}>{card}</div>);
  const cardStyle = {
    display: 'flex',
    flexDirection: 'row',
    flexWrap: 'wrap',
